...
export const AddFlockModal: React.FC<AddFlockModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        'Flock Registered',
        `Flock "${flockName.trim()}" (${initialCount.toLocaleString()} birds) added successfully.`,
        'success',
        4000
      );
      ...
      showToast('Flock Save Error', err.message || 'Failed to create flock.', 'error', 5000);
    } finally {
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-emerald-500"
              required
            />
          </div>
